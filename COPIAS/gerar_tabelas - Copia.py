...
def tabela_movimentacoes_mensais(parametros_funcoes):
    ...
    # Delete unnecessary keys (mercados, modalidades, and total)
    # for investment in movimentacoes_mensais_reduzido.copy():
    #     if investment in skip_investment_list or investment == "TOTAL":
    #         del movimentacoes_mensais_reduzido[investment]

    movimentacoes_mensais_reduzido = {
        k: v for k, v in movimentacoes_mensais_reduzido.items()
        if k not in skip_investment_list or k != "TOTAL"
    }
    ...
    for investment, value in movimentacoes_mensais_reduzido.items():
        # Ensure values are numeric before comparing
        ...
        try:
            ...
            saida_value = 0
        # Skip investments with zero values for both entrada and saida
        ...
        if month_entrada_key in value and month_saida_key in value:
            print("movimentacoes_mensais_reduzido[investment][month_bruto_key] =", value[month_entrada_key])
            print("movimentacoes_mensais_reduzido[investment][month_liquido_key] =", value[month_saida_key])
            ...
            filtered_dict_investimentos[investment][month_entrada_key] = 0 if month_entrada_key not in value else \
            value[
                month_entrada_key]
            filtered_dict_investimentos[investment][month_saida_key] = 0 if month_saida_key not in value else value[
                month_saida_key]
...
def construir_imagem_tabela(parametros_funcoes, tipo_tabela, filtered_reduced_dict):
    ...
    if tipo_tabela == "TABELA VALORES MENSAIS INVESTIMENTOS":
    ...
    elif tipo_tabela == "TABELA EVOLUCAO PATRIMONIAL INVESTIMENTOS":
        sort_key_evolucao_investimentos = month_interest
        # Create a NumPy array from filtered_dict
        numpy_arrays = (
            centralized_imports.gerar_graficos.criar_matriz_numpy(filtered_reduced_dict,
        ...
        df_reduzido = df_reduzido.applymap(lambda x: centralized_imports.locale.currency(x, grouping=True))
        # Save the table as an image
        image_filepath = r"C:\Users\ridim\OneDrive\FINANCE\FINANCAS_INVESTIMENTOS\RELATORIOS\evol_patr_invest_table.png"
        # Adjust column widths manually (starting with 0 for the first actual column)
