...
#     valid_months = centralized_imports.investimentos_btg.months_list[:month_interest_numeric]  # Restrict valid months
#
#     print(f"Processing data up to {year_month_interest_end} (year: {year_interest}, months: {valid_months})")
#
#     # Load files
#     try:
#         variacoes_patrimoniais_mensais = centralized_imports.arquivos_functions.ArquivosFunctions.abrir_arquivo_pickle(
...
#         variacoes_patrimoniais_anuais = {}
#
#     # Iterate over investments and sum the values up to year_month_interest_end
...
#
#         # Convert "DATA COMPRA" string to datetime
#         data_compra_str = variacoes_patrimoniais_mensais[investment]["DATA COMPRA"]
#         try:
#             data_compra = centralized_imports.datetime.datetime.strptime(data_compra_str, "%d/%m/%Y")  # Assuming the format is "DD/MM/YYYY"
#         except ValueError as e:
#             print(f"Error parsing 'DATA COMPRA' for {investment}: {e}")
#             continue  # Skip this investment if the date is invalid
#
#         total_for_year = 0.0
#         total_all_years = 0.0
#
...
#                     key = f"{month.upper()} {year_str}"
#                     total_for_year += variacoes_patrimoniais_mensais[investment].get(key, 0.0)
#             else:
...
#         variacoes_patrimoniais_anuais[investment] = {}
    ...
    # data_compra_str = variacoes_patrimoniais_mensais[investment]["DATA COMPRA"]
    # try:
    #     data_compra = centralized_imports.datetime.datetime.strptime(data_compra_str, "%Y-%m-%d")  # Adjust to correct format
    # except ValueError as e:
    #     print(f"Error parsing 'DATA COMPRA' for {investment}: {e}")
    #     continue
    ...
    #     year_str = str(year)
        ...
        # if year == year_interest:
            # Sum values only up to the selected month in the current year
            # for month in valid_months:
                # Check both short and full key format
                # key_short = f"{month.upper()}"
                # key_full = f"{month.upper()} {year_str}"
