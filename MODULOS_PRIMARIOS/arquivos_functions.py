...
class ArquivosFunctions:
    ...
    # Function #2: Exibir lista pickle
    # -------------------------------------------------------------
    @staticmethod
    def exibir_lista_pickle(filepath_pickle):
        try:
            # Open the pickle file in read-binary mode
            with open(filepath_pickle, 'rb') as file:
                # Load the list from the file
                data_list = centralized_imports.pickle.load(file)

                # Check if the loaded data is a list
                if isinstance(data_list, list):
                    print("Here is the list saved in the file:")
                    print()
                    for item in data_list:
                        print(item)
                else:
                    print("Error: The file does not contain a list.")

        except FileNotFoundError:
            print(f"Error: The file '{filepath_pickle}' was not found.")
        except centralized_imports.pickle.UnpicklingError:
            print("Error: The file could not be loaded as a pickle file.")
        except Exception as e:
            print(f"An unexpected error occurred: {e}")


    # Function #2: Exibir arquivo (dicionario) pickle
    # --------------------------------------------------------------------
    @staticmethod
