...
def atualizar_valores_mensais(config):
    ...
    if input_mode == "automatic":
        ...
        filepath_variacoes_patrimoniais_mensais = dicionario_palavras_chave.get(
        ...
        month_year_label = dicionario_palavras_chave.get("month_year_label")
    ...
    # print("month_year_label =", month_year_label)
    ...
    filepath_base_dados_investimentos = filepaths_dictionary.get("filepath_base_dados_investimentos")
    filepath_dados_financeiros_historicos = filepaths_dictionary.get("filepath_dados_financeiros_historicos")
    filepath_movimentacoes_anuais = filepaths_dictionary.get("filepath_movimentacoes_anuais")
    filepath_movimentacoes_mensais_padrao = filepaths_dictionary.get("filepath_movimentacoes_mensais_padrao")
    filepath_rentabilidade_representatividade = filepaths_dictionary.get("filepath_rentabilidade_representatividade")
    filepath_valores_mensais_padrao = filepaths_dictionary.get("filepath_valores_mensais_padrao")
    ...
    parametros_funcoes = {
        "filepath_base_dados_investimentos": filepath_base_dados_investimentos,
    ...
    while True:
        ...
        print("1. Criar ou atualizar base de dados de investimentos")
        print("2. Atualizar Rentabilidade e Representatividade")
        print("3. Exibir base de dados de investimentos")
        print("4. Criar dicionarios padronizados")
        ...
        if choice == '1':
            centralized_imports.criar_dicionarios_padronizados.criar_base_dados_investimentos(parametros_funcoes)
        elif choice == '2':
            ...
            centralized_imports.arquivos_functions.ArquivosFunctions.exibir_lista_pickle(filepath_base_dados_investimentos)
        elif choice == '4':
            centralized_imports.criar_dicionarios_padronizados.criar_dicionarios_padrao(parametros_funcoes)
        elif choice == '5':
        ...
        try:
            ...
            for index, investment in enumerate(investment_list, start=1):
                ...
                if data_compra and data_vencimento:
                    ...
                    if maturity_date < year_month_interest_start:
                        valores_mensais[investment][month_bruto_key] = 0
                        valores_mensais[investment][month_liquido_key] = 0
                        print(f"{investment} is already due. Setting values to zero and skipping...")
                        continue
    ...
    # Conferir com padrao e base de dados
    valores_mensais_padrao = centralized_imports.arquivos_functions.ArquivosFunctions.abrir_arquivo_pickle(filepath_valores_mensais_padrao)
    base_dados_investimentos = centralized_imports.arquivos_functions.ArquivosFunctions.abrir_arquivo_pickle(filepath_base_dados_investimentos)
    centralized_imports.criar_dicionarios_padronizados.atualizar_dicionario_alvo(
        valores_mensais_padrao,valores_mensais,filepath_valores_mensais)
    centralized_imports.criar_dicionarios_padronizados.atualizar_dicionario_alvo(
        base_dados_investimentos, valores_mensais, filepath_valores_mensais)
    ...
    if input_mode == "automatic":
        ...
        print("Using config value for gerar_dados_historicos:", pergunta7)
    else:
        # pergunta7 = input("Deseja gerar_dados_historicos agora? (s/n): ")
        pergunta7 = "s"
        print("You entered:", pergunta7)
    if pergunta7.lower() == 's':
        print("CALLING THE gerar_dados_historicos FUNCTION.")
        centralized_imports.gerar_dados_historicos.gerar_dados_historicos(parametros_funcoes)

    # --------------------------------------------------------------------------------
    print("-" * 90)
    if input_mode == "automatic":
        pergunta8 = config.get("pergunta8", "n")
        print("Deseja gerar_relatorio_pdf agora? (s/n):", pergunta8)
    else:
        # pergunta8 = input("Deseja gerar_relatorio_pdf agora? (s/n): ")
        pergunta8 = "s"
        print("You entered:", pergunta8)
    if pergunta8.lower() == 's':
